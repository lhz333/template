  ...
  <div class="crud-opts">
    <span class="crud-opts-left">
      ...
      <upload-file
        ...
        class="filter-item"
        :importAndDownload="crud.optShow.importAndDownload"
        :action="crud.importUrl"
        :btnTextName="crud.importText"
        :uploadParams="crud.importParams"
        @uploadSuccess="crud.refresh"
      ></upload-file>
      <el-button
        v-if="crud.optShow.template"
        :loading="crud.downloadTplLoading"
        class="filter-item"
        type="info"
        icon="el-icon-download"
        @click="crud.doTemplate"
      >{{crud.templateText}}</el-button>
      <el-button
        v-if="crud.optShow.download"
        :loading="crud.downloadLoading"
        :disabled="!crud.data.length && crud.downloadIsChecked"
        class="filter-item"
        size="mini"
        type="warning"
        icon="el-icon-download"
        @click="crud.doExport"
      >{{crud.downloadText}}</el-button>
      <!--右侧-->
      <slot name="right" />
    </span>
    <el-button-group class="crud-opts-right">
      <el-button
        v-if="crud.rightShow.search"
        size="mini"
        plain
        type="info"
        icon="el-icon-search"
      ...
      />
      <el-button
        v-if="crud.rightShow.refresh"
        size="mini"
        icon="el-icon-refresh"
        @click="crud.refresh()"
      />
      <el-popover v-if="crud.rightShow.filter" placement="bottom-end" width="150" trigger="click">
