...
function CRUD (options) {
  const defaultOptions = {
    ...
    // 重置表单
    defaultForm: () => { },
    // 排序规则，默认 id 降序 sortName排序名称 sortOrder 1升序 -1降序
    sortName: 'id',
    sortOrder: -1,
    // 等待时间
    time: 50,
  ...
  const methods = {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          if (data.data.Status == 'false') {
            crud.errorNotify(data.data.Result || data.data.Msg)
          }
          ...
          crud.page.total = data.data.Result.total || data.data.Result.totalElements
          crud.data = data.data.Result.content || []
    ...
    doAdd () {
      ...
      crud.crudMethod.add(crud.form).then((res) => {
        if (res.data.Status == 'true') {
          ...
          crud.errorNotify(res.data.Result || res.data.Msg)
          crud.status.add = CRUD.STATUS.PREPARED
     ...
     */
    doExport () {
      if (crud.page.total == 0 && crud.downloadIsChecked) {
      ...
      download(crud.downloadUrl, params).then(result => {
        //如果请求头里返回文件名，则直接使用请求头的文件名；若没，则使用页面中设置的
        let fileName = result.headers['content-disposition'] ? result.headers['content-disposition'].split('=')[1]
          : `${crud.title}数据.xlsx`
    ...
    */
    doTemplate () {
      crud.downloadTplLoading = true
      download(crud.templateUrl).then(result => {
        //如果请求头里返回文件名，则直接使用请求头的文件名；若没，则使用页面中设置的
        let fileName = result.headers['content-disposition'] ? result.headers['content-disposition'].split('=')[1]
          : `${crud.title}数据.xlsx`
        downloadFile(result, fileName)
        crud.downloadTplLoading = false
      }).catch(() => {
        crud.downloadTplLoading = false
      })
    },
    ...
    getQueryParams: function () {
      // 清除参数无值的情况
      Object.keys(crud.query).length && Object.keys(crud.query).forEach(item => {
        if (crud.query[item] === null || crud.query[item] === '') crud.query[item] = undefined
      })
      Object.keys(crud.params).length && Object.keys(crud.params).forEach(item => {
        if (crud.params[item] === null || crud.params[item] === '') crud.params[item] = undefined
      })
      return {
        pageNumber: crud.page.page,
        pageSize: crud.page.size,
        sortName: crud.sortName,
    ...
    // 排序
    sortByOrder (column) {
      let sortOrder = column.order === "ascending" ? 1 : -1
      let sortName = column.prop
      crud.sortOrder = sortOrder
      if (sortName) {
        crud.sortName = sortName
      }
      crud.refresh()
    },
    // 选择改变
    ...
    resetQuery (toQuery = true) {
      ...
      // 重置参数
      this.params = {}
      if (crud.findVM().$refs['datePicker']) {
        crud.findVM().$refs['datePicker'].clear()
      }
    ...
    getTable () {
      let table = this.findVM('presenter').$refs.table.$refs.hasOwnProperty('children') ? this.findVM('presenter').$refs.table.$refs['children'] : this.findVM('presenter').$refs.table
      return table
    },
    attchTable () {
